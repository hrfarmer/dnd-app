    ...
    <div className="flex flex-col-reverse h-full">
      ...
      >
        <input
          ...
          value={inputValue}
          onInput={(i: React.ChangeEvent<HTMLInputElement>) => setInputValue(i.target.value)}
        />
      ...
      <div className="h-[calc(100vh-8rem)] p-2 flex gap-2 flex-col-reverse overflow-y-auto">
        {state.messages.reverse().map((message, key) => {
          const user = state.connectedUsers.find(([userString]) => userString === message.author)

          if (!user) {
            return <div>Invalid user for some reason</div>
          }
          return <ChatMessage user={user[1]} key={key} message={message.content} />
        })}
      </div>
    </div>
  )
